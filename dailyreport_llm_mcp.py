...
- `summary`: 一条已经计算好的“汇总”记录，字段含义与 `records` 中完全一致，其中：
  - `total` / `solved` / `satisfied` 为所有街道数据的求和结果；
  - `solved_rate` / `satisfied_rate` / `score` 为所有街道数据的算术平均值。
